...
/** kernel.elf 等のファイルを読み取る処理
 * ファイルを渡すと，そのファイルの大きさを読み取ってその分のメモリ領域を割り当てる
 * 割り当てたメモリ領域は第二引数に書き戻される
 * fat_disk が用意されていない場合はelse節を実行する
 * Block I/O Protocol を取得・利用することで，ブロックデバイスの情報を得たり，ブロック単位での書き込みを行える
 * block_io->Media に関連付けられているブロックデバイスの各種情報が記録されている
 * BlockSize と LastBlock を利用することでブロックの byte 単位での全体サイズを求める
 */
                           ...
                           EFI_SYSTEM_TABLE *system_table) 
{
    ...
    entry_point(&config, &memmap, acpi_table, volume_image); //entry_pointを呼び出し(entry_addrから実行)．
