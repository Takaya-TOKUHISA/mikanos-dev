...
void Terminal::MoveCursor(uint8_t modifier, int direction) {
    if (cursor_.x + direction >= 1 && cursor_.x + direction < kColumns && cursor_.x + direction <= linebuf_index_) {
        if (modifier & (kLShiftBitMask | kRShiftBitMask)) {
            clip_area_.SelectArea(cursor_.x, direction);
        } else {
            clip_area_.FreeArea();
        }
        cursor_.x += direction;
...
void Terminal::ExecuteLine() {
    ...
    if (redir_char) {
        *redir_char = 0;
        ...
        /* リダイレクトがあればfiles_[1]が通常標準出力がターミナルであるところを指定されたファイルに切り替える */
    ...
    if (pipe_char) {
        *pipe_char = 0;
        ...
        /** 新しいタスクを，subcommand命令，自動終了，非表示，標準入力は新しいターミナル
         * 標準出力・標準エラー出力は現在のターミナルと同一として設定する
         */
    ...
    if (pipe_fd) {
        ...
        exit_code = ec; // 右側の終了コードを全体の終了コードとする
    }
