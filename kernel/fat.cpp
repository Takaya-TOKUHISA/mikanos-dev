    ...
    FindFile(const char* path, unsigned long directory_cluster) {
        ...
        while (directory_cluster != kEndOfClusterchain) {
            ...
            for (int i = 0; i < bytes_per_cluster / sizeof(DirectoryEntry); ++i) {
                ...
                /* それがディレクトリなら自身を呼び出す再帰的な関数として処理する */
