...
namespace {
    ...
    /* 最上位テーブル(PageMapLevel4table):PDPテーブルの変換テーブル */
    alignas(kPageSize4K) std::array<uint64_t, 512> pml4_table; 
    /* PageDirectoryPointertable: ページディレクトリの変換テーブル */
    alignas(kPageSize4K) std::array<uint64_t, 512> pdp_table;
    /* Page_direcroty: ページテーブルの変換テーブル */
    alignas(kPageSize4K)
...
/** 原因となったページに物理フレームを割り振る
 * P -> 0: 存在しないページ
 *      1: ページの権限違反
 * W/R->0: 読み込み例外
 *      1: 書き込み例外  
 * U/S->0: スーパーバイザーモードのアクセス
 *      1: ユーザーモードのアクセス
 * RSVD->0: 予約ビットの違反が例外の原因でない
 *       1: 予約ビットが1になっているため
 */
Error HandlePageFault(uint64_t error_code, uint64_t causal_addr) {
    auto& task = task_manager->CurrentTask();
    /* ページは存在するけど，権限違反で例外が発生した場合 */
    if (error_code & 1) {
        return MAKE_ERROR(Error::kAlreadyAllocated);
    }
    /** ページフォールトの原因アドレスがデマンドページングの有効な範囲に収まっているか確認
     * バッファオーバランなどのバグでのメモリ枯渇を防ぐ 
     */
